...
describe('MovieService', () => {
  ...
  describe('findRecent', () => {
    ...
    it('should fetch recent movies from the repository and cache them if not found in cache', async () => {
      ...
      expect(res).toEqual(recentMovies);
    });
  });
  ...
  describe('findAll', () => {
    ...
    beforeEach(() => {
      ...
      getLikedMovieMock = jest.spyOn(movieService, 'getLikesMovies');
    });
    it('should return a list of movies without user lies', async () => {
      const movies = [
        {
          id: 1,
          title: 'test',
      ...
      const dto = { title: 'test2' } as GetMoviesDto;
      const qb: any = {
      ...
      const res = await movieService.findAll(dto);
    ...
    it('should return a list of movies with user likes', async () => {
      ...
      const userId = 1;
      const res = await movieService.findAll(dto, userId);
  ...
  describe('findOne', () => {
    ...
    it('should return a movie if found', async () => {
      ...
      const res = await movieService.findOne(1);
  ...
  describe('create', () => {
    ...
    it('should create a movie successfully', async () => {
      ...
      const res = await movieService.create(createMovieDto, userId, qr);
    ...
    it('should throw NotFoundExcetion if director does not exist', async () => {
      const createMovieDto: CreateMovieDto = {
        title: 'new Movie',
        directorId: 1,
        genreIds: [1, 2],
  ...
  describe('update', () => {
    ...
    it('should update a movie successfully', async () => {
      ...
      const director = { id: 1, name: 'test2' };
      const genres = [
        {
          id: 1,
          name: 'genre 1',
      ...
      const res = await movieService.update(1, updateMovieDto);
    ...
    it('should throw NotFoundExceptoin if movie dose not exist', async () => {
      const updateMovieDto: UpdateMovieDto = {
        title: 'updated movie',
      };
    ...
    it('should throw NotFoundException if new genres do not exist', async () => {
      ...
      expect(qr.manager.find).toHaveBeenCalledWith(Genre, {
        where: { id: In(updateMoviesDto.genreIds as number[]) },
      });
      expect(qr.rollbackTransaction).toHaveBeenCalled();
    });
  ...
  describe('remove', () => {
    ...
    it('should remove a movie successfully', async () => {
      ...
      const res = await movieService.remove(1);
  ...
  describe('toggleMovieLike', () => {
    ...
    let updatedLikeMock: jest.SpyInstance;
    ...
    beforeEach(() => {
      ...
      updatedLikeMock = jest.spyOn(movieUserLikeRepository, 'update');
    ...
    it('should toggle movie like status successfully when like record exists and isLike is different', async () => {
      ...
      const res = await movieService.toggleMovieLike(1, 1, false);
      ...
      expect(updatedLikeMock).toHaveBeenCalledWith(
        {
          movie,
          user,
        },
      ...
      expect(res).toEqual({ isLike: false });
    });

    it('should remove like record when isLike is the same as the existing record', async () => {
      const movie = { id: 1 };
      const user = { id: 1 };
      const likeRecord = { movie, user, isLike: true };
      ...
      const res = await movieService.toggleMovieLike(1, 1, true);
      ...
      expect(res).toEqual({ isLike: null });
    });

    it('should save a new like record when no existing record found', async () => {
      const movie = { id: 1 };
      const user = { id: 1 };
      ...
      expect(res).toEqual({ isLike: true });
    });
